...
function s.tdfilter(c)
    return c:IsCode(CARD_TEMPLE_OF_THE_KINGS) or c:IsTrap() and c:IsAbleToDeckAsCost()
end
function s.rmfilter(c)
    return (c:IsCode(CARD_TEMPLE_OF_THE_KINGS) or c:IsTrap()) and c:IsAbleToRemoveAsCost()
end
function s.spcon(e,c)
    ...
    local g2=Duel.GetMatchingGroup(s.rmfilter,tp,LOCATION_GRAVE,0,nil)
	return g:GetClassCount(Card.GetCode)>=3 and g1:GetCount()>=1 and g2:GetCount()>=1
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,c)
	local rg=Duel.GetMatchingGroup(s.tdfilter,tp,LOCATION_GRAVE,0,c)
	local g=aux.SelectUnselectGroup(rg,e,tp,2,2,aux.dncheck,1,tp,HINTMSG_SELECT,nil,nil,true)
	if #g>0 then
		g:KeepAlive()
		e:SetLabelObject(g)
		return true
