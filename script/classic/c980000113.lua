...
function s.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_MADOLCHE)
end
...
function s.filter(c)
	return c:IsCode(14001430) and c:IsAbleToHand()
end
...
function s.shop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,e:GetHandler()) then return end
