...
function s.initial_effect(c)
	...
	--This card gains ATK equal to the combined Ranks of all Xyz Monsters currently on your field x 100
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and c:IsFaceup() then
		local rks=Duel.GetMatchingGroup(Card.IsFaceup,tp,LOCATION_MZONE,0,nil):GetSum(Card.GetRank)
		if rks>0 then
			--This card gains ATK equal to the combined Ranks of all Xyz Monsters currently on your field x 100, during that damage calculation only
			c:UpdateAttack(rks*100,RESET_EVENT|RESETS_STANDARD_DISABLE|RESET_PHASE|PHASE_DAMAGE_CAL)
		end
	end
end
