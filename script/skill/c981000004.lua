...
function s.revfilter(c,tp)
    local lc=c.listed_names
    if lc == nil then return false end
	return c:IsSetCard({SET_NUMBER_C, SET_CXYZ}) and c:IsType(TYPE_XYZ) and not c:IsPublic()
         and Duel.IsExistingMatchingCard(s.listfilter, tp, LOCATION_MZONE, 0, 1, cost_card,lc)
end
function s.listfilter(c,codes)
    return c:IsCode(codes)
end
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local rc=Duel.SelectMatchingCard(tp, s.revfilter, tp, LOCATION_EXTRA, 0, 1, 1)
    Duel.ConfirmCards(1-tp, rc)
    Duel.ShuffleExtra(tp)
    --Shuffle a card to the Deck and add Barian's Force
    Duel.Hint(HINT_SELECTMSG, tp, HINTMSG_TODECK)
    local g=Duel.SelectMatchingCard(tp,Card.IsAbleToDeck,tp,LOCATION_HAND,0,1,1,nil, p, cost_card):GetFirst()
